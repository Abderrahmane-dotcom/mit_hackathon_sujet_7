...
"""
    ...
    # Live UI container
    st.subheader("🧪 Intermediate Results (Live)")
    ...
    for t, frame_name in enumerate(image_files):
        ...
        for idx, mask in enumerate(sam_output["masks"]):
            ...
            output = vlm(
                text=messages,
                max_new_tokens=120,
                max_length=None,     # ✅ fixes the warning
                do_sample=False,
                temperature=0.0,
        ...
        with live_container:
            ...
            with col1:
                st.markdown(f"### ⏱️ Time {t}s")
                st.json(frame_result)

            with col2:
                if os.path.exists(out_path):
                    st.image(
                        out_path,
                        caption=f"Annotated frame @ {t}s",
    ...
    st.subheader("🖼️ All Annotated Frames")
    st.image(
